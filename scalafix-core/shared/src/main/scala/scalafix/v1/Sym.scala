...
object Sym {
  ...
  final class Kind private[Sym] (info: s.SymbolInformation) {
    ...
    def isDef: Boolean = k.isMethod && !is(p.VAL) && !is(p.VAR)
    def isVal: Boolean = k.isMethod && is(p.VAL)
    def isVar: Boolean = k.isMethod && is(p.VAR) && !isSetter
    def isVarSetter: Boolean = k.isMethod && is(p.VAR) && isSetter
  ...
  final class Accessibility private[Sym] (a: s.Accessibility) {
    ...
    def within: Option[Sym] =
      if (a.symbol.isEmpty) scala.None
      else Some(new Sym(a.symbol))
  }
